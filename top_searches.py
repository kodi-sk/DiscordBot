...
def google_output(username, query):  # Method for inserting input into DB and getting top searches from google.
    number_of_results = 5
    timeout_google = 2
    lang = 'en'
    tld = "co.in"
    insert_in_db(username, query)
    return get_google_query_list(query, tld, lang, number_of_results, timeout_google)


def insert_in_db(username, query):  # insert the input in DB for future reference
    db_table_name = 'google_query'
    user_input = {'username': username, 'input': query, 'created_at': datetime.datetime.now()}
    print(user_input)
    try:
